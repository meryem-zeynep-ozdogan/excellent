...
class InvoiceExcelExporter:
    ...
    def _auto_adjust_column_widths(self, writer, sheet_name, df):
        ...
        try:
            ...
            money_format = workbook.add_format(
                {"num_format": "#,##0.00", "valign": "top", "border": 1}
            )
            ...
            for i, col in enumerate(df.columns):
                ...
                if len(df) > 0:
                    for row_num in range(1, len(df) + 1):
                        ...
                        if col in ["TUTAR (TL)", "TUTAR", "MİKTAR"] and isinstance(
    ...
    def _prepare_invoice_data(self, invoice_data, lang="tr"):
        ...
        for invoice in invoice_data:
            ...
            usd_text = (
                f"{matrah_usd:,.2f}"
                if usd_rate_val == 0
                else f"{matrah_usd:,.2f} ({usd_rate_val:.2f} {tr('currency_tl', lang)})"
            )
            eur_text = (
                f"{matrah_eur:,.2f}"
                if eur_rate_val == 0
                else f"{matrah_eur:,.2f} ({eur_rate_val:.2f} {tr('currency_tl', lang)})"
            )
    ...
    def export_invoices_to_excel(
        self, invoice_data, invoice_type, file_path=None, lang="tr"
    ):
        ...
        try:
            ...
            sheet_name = (
                tr("excel_sheet_outgoing", lang)
                if invoice_type == "outgoing"
                else tr("excel_sheet_incoming", lang)
            )
    ...
    def export_general_expenses_to_excel(self, expense_data, file_path=None, lang="tr"):
        ...
        try:
            ...
            sheets_data = {
                tr("excel_sheet_general_expenses", lang): {"data": excel_data}
            }
            return self.export_to_excel(file_path, sheets_data)
...
def export_monthly_general_expenses_to_excel(
    ...
    try:
        ...
        worksheet = workbook.add_worksheet(
            f"{year} {tr('excel_sheet_general_expenses', lang)}"
        )
        ...
        workbook.close()
        logging.info(
            f"Aylık genel giderler '{os.path.basename(file_path)}' dosyasına aktarıldı."
        )
        return True
...
def export_monthly_income_to_excel(
    ...
    try:
        ...
        for color_name, color_code in colors.items():
            month_formats[color_name] = workbook.add_format(
                {**month_format_base, "bg_color": color_code}
            )
        ...
        for color_name, color_code in colors.items():
            money_formats[color_name] = workbook.add_format(
                {**money_format_base, "bg_color": color_code}
            )
        ...
        for color_name, color_code in colors.items():
            kdv_formats[color_name] = workbook.add_format(
                {**kdv_format_base, "bg_color": color_code}
            )
        ...
        for color_name, color_code in colors.items():
            percent_formats[color_name] = workbook.add_format(
                {**percent_format_base, "bg_color": color_code}
            )
        ...
        for i, month_name in enumerate(months):
            ...
            worksheet.write(
                current_row,
                4,
                kurumlar_yuzde / 100 if kurumlar_yuzde else 0,
                percent_formats[color],
            )
            ...
            if i % 3 == 0:
                ...
                worksheet.merge_range(
                    current_row, 5, current_row + 5, 5, odenecek_vergi, merged_format
                )
