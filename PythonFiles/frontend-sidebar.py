...
if last_error == winerror.ERROR_ALREADY_EXISTS:
    ...
    ctypes.windll.user32.MessageBoxW(
        0, get_text("app_already_running", "tr"), get_text("warning", "tr"), 0x30
    )
    sys.exit(0)
    ...
    expense_buttons = ft.Container(
        ...
        content=ft.Row(
            [
                ...
                ft.Container(height=38, content=currency_dropdown),
                btn_save,
                btn_excel,
                btn_pdf,
            ],
...
def main(page: ft.Page):
        ...
        if e.path:
            ...
            if invoices:
                ...
                if success:
                    ...
                    page.snack_bar = ft.SnackBar(
                        content=ft.Text(tr("msg_excel_export_error"), color=col_white),
                        bgcolor=col_danger,
                    )
                    page.snack_bar.open = True
                    page.update()
        ...
        if e.path:
            ...
            if invoices:
                ...
                if success:
                    ...
                    page.snack_bar = ft.SnackBar(
                        content=ft.Text(tr("msg_pdf_export_error"), color=col_white),
                        bgcolor=col_danger,
                    )
                    page.snack_bar.open = True
                    page.update()
    ...
    class SidebarButton(ft.Container):
        def __init__(self, icon_name, text, page_name, is_selected=False):
            ...
            self.text_control = ft.Text(
                ...
                size=15,
                weight="w600",
                visible=state["sidebar_expanded"],
                color=initial_color,
            )
            self.content_row = ft.Row(
                [self.icon_expanded, self.text_control],
                spacing=15,
                alignment=ft.MainAxisAlignment.START,
                visible=state["sidebar_expanded"],
            )
            self.content_icon_only = ft.Container(
                content=self.icon_collapsed,
                alignment=ft.alignment.center,
                visible=not state["sidebar_expanded"],
            )
        ...
        def update_visuals(self, run_update=True):
            ...
            if self.is_selected:
                ...
                new_color = "onPrimary"
                self.shadow = ft.BoxShadow(
                    blur_radius=10, color="shadow", offset=ft.Offset(0, 4)
                )
            else:
            ...
            if isinstance(col, ft.Column):
                for btn in col.controls:
                    if isinstance(btn, SidebarButton):
                        ...
                        btn.padding = (
                            ft.padding.only(left=15) if state["sidebar_expanded"] else 0
                        )
                        btn.alignment = (
                            ft.alignment.center_left
                            if state["sidebar_expanded"]
    ...
    def create_donemsel_page():
        ...
        def export_to_excel_donemsel(e):
            ...
            try:
                ...
                save_file_picker_excel.save_file(
                    dialog_title=tr("title_save_excel_report"),
                    file_name=filename,
                    allowed_extensions=["xlsx"],
                )
            except Exception as ex:
        ...
        def export_to_pdf_donemsel(e):
            ...
            try:
                ...
                save_file_picker_pdf.save_file(
                    dialog_title=tr("title_save_pdf_report"),
                    file_name=filename,
                    allowed_extensions=["pdf"],
                )
            except Exception as ex:
        ...
        right_buttons = ft.Container(
            ...
            content=ft.Row(
                [
                    create_styled_icon_button(
                        ft.Icons.TABLE_VIEW,
                        "#217346",
                        "Excel",
                        export_to_excel_donemsel,
                    ),
                    create_styled_icon_button(
                        ft.Icons.PICTURE_AS_PDF,
                        "#D32F2F",
                        "PDF",
                        export_to_pdf_donemsel,
                    ),
                ],
                spacing=8,
            ),
        ...
        top_bar = ft.Row(
            [ft.Container(height=38, content=year_dropdown), right_buttons],
            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
        )
    ...
    def create_invoices_page():
                    ...
                    if selected_count > 0:
                        ...
                        if selected_count == 1 and isinstance(
                            ...
                            if matrah and matrah > 0:
                                ...
                                input_tutar.value = (
                                    str(round(tutar, 5)) if tutar else "0"
                                )
                            ...
                            input_usd_kur.value = (
                                str(round(usd_rate, 5))
                                if usd_rate and usd_rate > 0
                                else ""
                            )
                            input_eur_kur.value = (
                                str(round(eur_rate, 5))
        ...
        def add_invoice(e):
            ...
            try:
                ...
                if not input_fatura_no.value or not input_fatura_no.value.strip():
                    error_label_fatura_no.tooltip = (
                        f"{tr('invoice_no')} {tr('cannot_be_empty')}"
                    )
                ...
                if not input_tutar.value or not input_tutar.value.strip():
                    ...
                    try:
                        ...
                        if tutar_val < 0:
                            error_label_tutar.tooltip = (
                                f"{tr('total')} {tr('cannot_be_negative')}"
                            )
                            error_label_tutar.visible = True
                            has_error = True
                    except ValueError:
                        error_label_tutar.tooltip = (
                            f"{tr('total')} {tr('must_be_number')}"
                        )
                        error_label_tutar.visible = True
                        has_error = True
            ...
            try:
                ...
                def confirm_delete(e):
                    ...
                    try:
                        ...
                        page.snack_bar = ft.SnackBar(
                            content=ft.Text(
                                tr("msg_delete_error_prefix").format(str(ex)),
                                color=col_white,
                            ),
                            bgcolor=col_danger,
                        )
            ...
            try:
                ...
                def on_folder_selected(e: ft.FilePickerResultEvent):
                    try:
                        ...
                        page.snack_bar = ft.SnackBar(
                            content=ft.Text(
                                tr("dialog_error_prefix").format(str(dialog_error)),
                                color=col_white,
                            ),
                            bgcolor=col_danger,
                        )
        ...
        def backup_database(e):
            ...
            try:
                ...
                def on_save_result(e: ft.FilePickerResultEvent):
                    if e.path:
                        ...
                        if success:
                            ...
                            page.snack_bar = ft.SnackBar(
                                content=ft.Text(
                                    tr("msg_backup_error").format(msg), color=col_white
                                ),
                                bgcolor=col_danger,
                            )
                            page.snack_bar.open = True
                ...
                page.update()
                save_file_picker.save_file(
                    dialog_title=tr("backup_save_title"),
        ...
        def export_to_excel(e):
            ...
            try:
                ...
                if hasattr(page, "invoice_excel_picker"):
                    page.invoice_excel_picker.save_file(
                        dialog_title=tr("title_save_excel"),
                        file_name=filename,
                        allowed_extensions=["xlsx"],
                    )
                else:
                    invoice_excel_picker.save_file(
                        dialog_title=tr("title_save_excel"),
                        file_name=filename,
                        allowed_extensions=["xlsx"],
                    )
        ...
        def export_to_pdf(e):
            ...
            try:
                ...
                if hasattr(page, "invoice_pdf_picker"):
                    page.invoice_pdf_picker.save_file(
                        dialog_title=tr("title_save_pdf"),
                        file_name=filename,
                        allowed_extensions=["pdf"],
                    )
                else:
                    invoice_pdf_picker.save_file(
                        dialog_title=tr("title_save_pdf"),
                        file_name=filename,
                        allowed_extensions=["pdf"],
                    )
        ...
        operation_buttons = ft.Row(
            [btn_clear, btn_add, btn_update, btn_delete_container], spacing=15
        )
        ...
        right_buttons_container = ft.Container(
            content=right_buttons_row, padding=ft.padding.only(right=25)
        )
    ...
    logo_image = ft.Image(
        src=resource_path("logo.png"), width=40, height=40, fit=ft.ImageFit.CONTAIN
    )
    logo_text = ft.Text(tr("app_title"), size=24, weight="bold", color="onBackground")

    menu_icon = ft.IconButton(
        icon="menu", icon_color="onBackground", on_click=toggle_sidebar
    )
    ...
    menu_row = ft.Row(
        [menu_icon, logo_column], spacing=5, alignment=ft.MainAxisAlignment.CENTER
    )

    btn_home = SidebarButton(
        "home_rounded", tr("nav_home"), "home", False
    )  # Başlangıçta False
    btn_faturalar = SidebarButton(
        "receipt_long_rounded", tr("nav_invoices"), "faturalar"
    )
    ...
    sidebar_column = ft.Column(
        [
            ...
            ft.Column(
                [theme_btn, lang_btn, ft.Container(height=20)],
                spacing=15,
                horizontal_alignment=ft.CrossAxisAlignment.CENTER,
            ),
        ],
    ...
    sidebar_container = ft.Container(
        ...
        shadow=ft.BoxShadow(blur_radius=10, color="shadow"),
    )
    ...
    ft.Text(
        get_exchange_rate_display(), size=13, color="onSurfaceVariant", weight="w600"
    )
        ...
        try:
            ...
            income_invoices = (
                backend_instance.handle_invoice_operation(
                    operation="get", invoice_type="outgoing", limit=1000, offset=0
                )
                or []
            )
            ...
            expense_invoices = (
                backend_instance.handle_invoice_operation(
                    operation="get", invoice_type="incoming", limit=1000, offset=0
                )
                or []
            )
    ...
    def update_transactions(filter_date=None):
        ...
        if not filtered_data:
            transactions_column.controls.append(
                ft.Container(
                    content=ft.Text(tr("no_transactions"), color="onSurfaceVariant"),
                    alignment=ft.alignment.center,
                    padding=20,
                )
            )
                        ...
                        ft.Row(
                            [
                                ft.IconButton(
                                    icon="calendar_month",
                                    icon_color="onSurfaceVariant",
                                    tooltip=tr("tooltip_go_to_date"),
                                    on_click=open_date_dialog,
                                ),
                                ft.TextButton(
                                    tr("btn_latest_entries"),
                        ...
                        [
                            ft.Row(
                                [
                                    ...
                                    ft.Text(tr("legend_income"), size=12, color="grey"),
                                ],
                                spacing=5,
                            ),
                            ft.Row(
                                [
                                    ...
                                    ft.Text(
                                        tr("legend_expense"), size=12, color="grey"
                                    ),
                                ],
                                spacing=5,
                            ),
    ...
    def check_for_updates():
        ...
        try:
            ...
            if update_info and update_info.get("update_available", False):
                ...
                msg = tr("update_available_msg").format(
                    update_info.get("version", "Bilinmiyor"),
                    update_info.get("description", "No description"),
                )
