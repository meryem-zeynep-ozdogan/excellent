...
try:
    ...
    warnings.warn(
        "UYARI: 'rust_qr' modülü bulunamadı. Performans düşebilir.", ImportWarning
    )
# -----------------------------
...
class OptimizedQRProcessor:
    ...
    def extract_text_from_pdf(self, pdf_path):
        ...
        try:
            ...
            logging.warning(
                f"⚠️ PDF metin çıkarma hatası ({os.path.basename(pdf_path)}): {e}"
            )
            return ""
    ...
    def process_pdf(self, pdf_path):
        ...
        try:
            ...
            if is_likely_vector:
                ...
                result = self._try_pdf_with_dpi(
                    page, 150, "VEKTÖR-ROI-150", clip=roi_rect
                )
                if result:
                    doc.close()
                ...
                result = self._try_pdf_with_dpi(
                    page, 250, "TARAMA-ROI-250", clip=roi_rect
                )
                if result:
                    doc.close()
                    self.stats["stage2_medium"] += 1
    ...
    def _try_pdf_with_dpi(self, page, dpi, stage_name, clip=None):
        ...
        try:
            ...
            pix = page.get_pixmap(
                matrix=mat, colorspace=fitz.csGRAY, alpha=False, clip=clip
            )
            ...
            if qr_string:
                ...
                try:
                    ...
                    logging.debug(
                        f"   ✅ Rust (RAW) ile bulundu ({stage_name} - {dpi} DPI)"
                    )
            ...
            for row_y, row_words in rows.items():
                ...
                for word in row_words:
                    ...
                    if any(
                        ...
                        for keyword in ["mal", "hizmet", "açıklama", "malzeme", "ürün"]
                    ):
                ...
                for word in row_words:
                    ...
                    if any(
                        keyword in word_lower
                        for keyword in ["miktar", "adet", "qty", "quantity"]
                    ):
                        miktar_col_x = word["x"]
                        if not header_y:
                            header_y = row_y
            ...
            for row_y in sorted(rows.keys()):
                ...
                if malzeme_col_x and not info["malzeme"]:
                    malzeme_candidates = [
                        w
                        for w in row_words
                        if abs(w["x"] - malzeme_col_x)
                        < 80  # 80 piksel tolerance (artırıldı)
                        and len(w["text"]) > 3
                        and not re.match(r"^[\d\s\.\,\-\%\:]+$", w["text"])
                    ]
                    if malzeme_candidates:
                        # En yakın olanı al
                        malzeme_candidates.sort(
                            key=lambda w: abs(w["x"] - malzeme_col_x)
                        )
                ...
                if miktar_col_x and not info["miktar"]:
                    ...
                    sayi_candidates = [
                        ...
                        and not re.match(
                            r"^\d{3,4}$", w["text"]
                        )  # 654, 1234 gibi kanun numaralarını ele
                    ]
                    ...
                    karma_candidates = [
                        ...
                        and re.search(
                            r"[A-Za-zÇğİŞÜçşıöü]", w["text"]
                        )  # İçinde harf var
                        and self._is_valid_birim(
                            re.sub(r"[\d\s\.\,]+", "", w["text"]).strip()
                        )  # Harf kısmı geçerli birim
                        and not re.match(
                            r"^[A-Z]+\d+$", w["text"].upper()
                        )  # M2, M3 gibi pure birimleri çıkar
                        and not re.search(
                            r"kanun|madde|fıkra|bent|no:|sayı", w["text"], re.IGNORECASE
                        )  # Kanun ifadelerini ele
                    ]
                    ...
                    if karma_candidates:
                        ...
                        birim_part = (
                            re.sub(r"[\d\s\.\,]+", "", selected["text"]).strip().upper()
                        )
                    ...
                    if not miktar_result and sayi_candidates and birim_candidates:
                        ...
                        for sayi_w in sayi_candidates:
                            ...
                            try:
                                ...
                                for birim_w in birim_candidates:
                                    ...
                                    miktar_col_distance = abs(
                                        sayi_w["x"] - miktar_col_x
                                    )
                        ...
                        if best_pair:
                            ...
                            try:
                                float_val = float(sayi_temiz)
                                miktar_result = (
                                    f"{float_val:.0f} {birim_w['text'].upper().strip()}"
                                )
                            except Exception:
                                pass
                        ...
                        if numeric_values:
                            ...
                            malzeme_x = next(
                                (
                                    w["x"]
                                    for w in row_words
                                    if w["text"] == info["malzeme"]
                                ),
                                0,
                            )
                            numeric_values.sort(
                                key=lambda w: abs(w["x"] - malzeme_x), reverse=True
                            )
                            ...
                            for num_val in numeric_values:
                                ...
                                try:
                                    ...
                                    if float_val > 0:
                                        # ⭐ BİRİM BİLGİSİNİ BUL ⭐
                                        birim_text = self._find_birim_near_miktar(
                                            row_words,
                                            num_val,
                                            miktar_col_x
                                            if miktar_col_x
    ...
    def _is_valid_birim(self, text):
        ...
        if len(text_upper) >= 2:
            for unit in valid_units:
                if (
                    unit in text_upper and len(text_upper) <= len(unit) + 3
                ):  # Biraz ekstra tolerans
                    return True
    ...
    def _extract_malzeme_classic(self, lines):
        ...
        for i, line in enumerate(lines):
            ...
            if any(
                re.search(pattern, line_lower) for pattern in malzeme_header_patterns
            ):
                table_start_idx = i
        ...
        if table_start_idx is not None:
            ...
            for i in range(search_start, min(search_start + 30, len(lines))):
                ...
                if any(
                    ...
                    for keyword in ["fiyat", "tutar", "vergi", "kdv", "birim"]
                ):
                    continue
    ...
    def _extract_miktar_classic(self, lines):
        ...
        for i, line in enumerate(lines):
            ...
            if any(
                re.search(pattern, line_lower) for pattern in miktar_header_patterns
            ):
                miktar_column_idx = i
                break
    ...
    def process_file(self, file_path):
        ...
        try:
            ...
            fatura_no_from_filename = (
                fatura_no_extracted if fatura_no_extracted else file_name_without_ext
            )
            ...
            extracted_info = self.extract_info_from_text(
                pdf_text, file_name_without_ext, pdf_path=file_path
            )
            ...
            if pdf_text:
                ...
                if firma or amounts["toplam"] > 0:
                    ...
                    fallback_json = {
                        ...
                        "toplamTutar": amounts[
                            "toplam"
                        ],  # KDV dahil toplam (referans için)
                    ...
                    logging.warning(
                        "   ⚠️ PDF'den yeterli bilgi çıkarılamadı (firma veya tutar yok)"
                    )
            ...
            fatura_no_from_filename = (
                fatura_no_extracted
                if fatura_no_extracted
                else os.path.splitext(file_basename)[0]
            )
            return {
                "dosya_adi": file_basename,
                "dosya_yolu": file_path,
                ...
                if re.search(keyword, line_lower):
                    ...
                    for j in range(i, min(i + 3, len(lines))):
                        ...
                        invoice_match = re.search(
                            r"([A-Z]{3}\d{12,}|[A-Z0-9]{10,})", lines[j]
                        )
                        if invoice_match:
    ...
    def _extract_amount_from_text(self, pdf_text):
        ...
        if not detected_currency:
            ...
            eur_with_amount = len(
                re.findall(r"[\d.,]+\s*EUR\b", pdf_text, re.IGNORECASE)
            )
            usd_with_amount = len(
                re.findall(r"[\d.,]+\s*USD\b", pdf_text, re.IGNORECASE)
            )
            tl_with_amount = len(
                re.findall(r"[\d.,]+\s*(?:TL|TRY)\b", pdf_text, re.IGNORECASE)
            )
        ...
        if kdv_percent_match:
            ...
            amounts["kdv_yuzdesi"] = round(
                (amounts["kdv"] / amounts["matrah"]) * 100, 2
            )
    ...
    def process_qr_files_in_folder(
        ...
        if CONCURRENT_AVAILABLE:
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                future_to_file = {
                    executor.submit(self.process_file, fp): fp for fp in file_paths
                }
                ...
                for future in as_completed(future_to_file):
                    ...
                    try:
                        ...
                        logging.error(
                            f"❌ Dosya işleme hatası ({os.path.basename(file_path)}): {exc}"
                        )
                        results.append(
                            {
                    ...
                    if status_callback:
                        try:
                            ...
                            msg = tr("processing_progress", lang).format(
                                progress, completed_count, len(file_paths)
                            )
            ...
            for file_path in file_paths:
                try:
                    ...
                    logging.error(
                        f"❌ Dosya işleme hatası ({os.path.basename(file_path)}): {e}"
                    )
                    results.append(
                        {
                            "dosya_adi": os.path.basename(file_path),
        ...
        for result in results:
            if result.get("durum") != "BAŞARILI":
                try:
                    ...
                    if source_path and os.path.exists(source_path):
                        ...
                        if os.path.exists(dest_path):
                            ...
                            dest_path = os.path.join(
                                failed_dir, f"{base}_{timestamp}{ext}"
                            )
                    ...
                    logging.error(
                        f"   ❌ Dosya taşıma hatası ({result.get('dosya_adi')}): {e}"
                    )
    ...
    def process_qr_file_list(
        ...
        valid_paths = [
            ...
            if os.path.isfile(fp) and os.path.splitext(fp)[1].lower() in ALLOWED
        ]
        if not valid_paths:
            logging.warning("⚠️ İşlenebilir dosya bulunamadı")
            return []
        ...
        if CONCURRENT_AVAILABLE:
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_file = {
                    executor.submit(self.process_file, fp): fp for fp in valid_paths
                }
                for future in as_completed(future_to_file):
                    file_path = future_to_file[future]
                    try:
                    ...
                    if status_callback:
                        try:
                            ...
                            msg = tr("processing_progress", lang).format(
                                progress, completed_count, len(valid_paths)
                            )
                            if not status_callback(msg, progress):
                                executor.shutdown(wait=False, cancel_futures=True)
                                break
                ...
                try:
                    ...
                    if source_path and os.path.exists(source_path):
                        ...
                        if os.path.exists(dest_path):
                            base, ext = os.path.splitext(file_name)
                            dest_path = os.path.join(
                                failed_dir, f"{base}_{int(time.time())}{ext}"
                            )
                        shutil.move(source_path, dest_path)
                        result["dosya_yolu"] = dest_path
                        result["tasindi"] = True
...
class QRInvoiceIntegrator:
    ...
    def add_invoices_from_qr_data(self, qr_results, invoice_type):
        ...
        for i, result in enumerate(qr_results, 1):
            ...
            if result.get("durum") == "BAŞARILI":
                ...
                parsed_fields = self._parse_qr_to_invoice_fields(
                    qr_json, extracted_info, fatura_no_from_filename
                )
                ...
                if (
                    not parsed_fields.get("toplam_tutar")
                    or float(parsed_fields.get("toplam_tutar", 0)) <= 0
                ):
                    with lock:
                        failed_imports += 1
                        failed_files.append(dosya_yolu)
        ...
        def process_invoice(invoice_data):
            ...
            try:
                result = self.backend.handle_invoice_operation(
                    operation="add", invoice_type=invoice_type, data=parsed_fields
                )
                ...
                with lock:
                    ...
                    failed_files.append(dosya_yolu)
                    processing_details.append(
                        {"file": dosya_adi, "status": "BAŞARISIZ", "error": str(e)}
                    )
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [
                executor.submit(process_invoice, inv) for inv in prepared_invoices
            ]
        ...
        try:
            ...
            for db_type in ["outgoing", "incoming"]:
                invoices = self.backend.handle_invoice_operation(
                    operation="get", invoice_type=db_type, limit=None
                )
    ...
    def _save_unadded_invoice(
        self, dosya_yolu, dosya_adi, reason, qr_json=None, parsed_fields=None
    ):
    ...
    def _detect_invoice_type(self, qr_json, parsed_fields):
        ...
        tip_field = self._get_value_case_insensitive(
            qr_json, ["tip", "type", "senaryo", "invoiceType"]
        )
        ...
        if tip_field:
            ...
            if any(
                keyword in tip_upper
                for keyword in ["ALIS", "ALIŞ", "PURCHASE", "BUYING", "ALIM"]
            ):
                return "incoming"
    ...
    def _parse_qr_to_invoice_fields(
        self, qr_json, extracted_info=None, fatura_no_from_filename=""
    ):
        ...
        for key, value in qr_json.items():
            ...
            if any(
                word in key_lower
                for word in ["tutar", "amount", "total", "pay", "matrah", "tax", "kdv"]
            ):
        ...
        parsed["tarih"] = (
            self.backend.format_date(str(qr_tarih))
            if qr_tarih
        ...
        if not firma or (isinstance(firma, str) and firma.isdigit()):
            ...
            if extracted_info.get("firma"):
                ...
                # Alternatif alanlar
                firma = (
                    self._get_value_case_insensitive(
                        qr_json, ["satici", "alici", "vkn", "unvan"]
                    )
                    or "Firma Bilgisi Yok"
                )
        ...
        if extracted_info.get("malzeme"):
            ...
            if qr_malzeme and qr_malzeme not in [
                "SATIS",
                "ALIS",
                "EARSIV",
                "TICARIFATURA",
        ...
        # Para birimi
        birim = str(
            self._get_value_case_insensitive(qr_json, key_map["birim"]) or "TRY"
        ).upper()
        parsed["birim"] = "TL" if birim in ["TRY", "TRL"] else birim

        # Tutar hesaplamaları
        toplam = self._to_float(
            self._get_value_case_insensitive(qr_json, key_map["toplam_tutar"])
        )
        matrah = self._to_float(
            self._get_value_case_insensitive(qr_json, key_map["matrah"])
        )
        kdv_tutari = self._to_float(
            self._get_value_case_insensitive(qr_json, key_map["kdv_tutari"])
        )
        kdv_yuzdesi = self._to_float(
            self._get_value_case_insensitive(qr_json, key_map["kdv_yuzdesi"])
        )
        ...
        if matrah > 0 and toplam > 0:
            ...
            parsed["kdv_tutari"] = (
                kdv_tutari if kdv_tutari > 0 else round(toplam - matrah, 2)
            )
            ...
            if kdv_tutari > 0 or payable_key_present:
                ...
                parsed["kdv_tutari"] = (
                    kdv_tutari
                    if kdv_tutari > 0
                    else round(toplam - (toplam / (1 + parsed["kdv_yuzdesi"] / 100)), 2)
                )
            else:
            ...
            parsed["kdv_tutari"] = (
                kdv_tutari
                if kdv_tutari > 0
                else round(matrah * parsed["kdv_yuzdesi"] / 100, 2)
            )
        else:
            # Hiçbiri yok - QR JSON'dan herhangi bir sayısal değer bul
            logging.warning(
                "      ⚠️ Standart tutar alanları bulunamadı, alternatif arama yapılıyor..."
            )
            ...
            if possible_amounts:
                ...
                parsed["kdv_dahil"] = False
                parsed["kdv_tutari"] = round(
                    best_amount * parsed["kdv_yuzdesi"] / 100, 2
                )
            else:
                # Gerçekten hiçbir tutar yok
                logging.error("      ❌ QR'da hiçbir tutar bilgisi bulunamadı!")
                logging.error(
                    f"      📋 QR JSON: {json.dumps(qr_json, indent=2, ensure_ascii=False)}"
                )
    ...
    def _extract_amount_from_text(self, pdf_text):
        ...
        lines = pdf_text.split("\n")
        amounts = {"toplam": 0.0, "matrah": 0.0, "kdv": 0.0, "kdv_yuzdesi": 0.0}
