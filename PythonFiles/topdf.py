...
class InvoicePDFExporter:
    ...
    def _setup_custom_styles(self):
        ...
        if "Calibri-Turkish" in registered_fonts:
            ...
            turkish_font_bold = (
                "Arial-Bold-Turkish"
                if "Arial-Bold-Turkish" in registered_fonts
                else "Calibri-Turkish"
            )
    ...
    def export_invoices_to_pdf(self, invoice_data, invoice_type, file_path, lang="tr"):
        ...
        try:
            ...
            story = self._create_invoice_document_layout(
                invoice_data, invoice_type, lang
            )
    ...
    def _create_invoice_document_layout(self, invoice_data, invoice_type, lang="tr"):
        ...
        type_text = (
            tr("pdf_income_invoices", lang)
            if invoice_type == "outgoing"
            else tr("pdf_expense_invoices", lang)
        )
        type_color = (
            "#28a745" if invoice_type == "outgoing" else "#dc3545"
        )  # Yeşil / Kırmızı
        story.append(
            Paragraph(
                f'<font color="{type_color}"><b>{type_text}</b></font>',
                ParagraphStyle(
                    "TypeTitle",
                    fontName=turkish_font_bold,
                    fontSize=10,
        ...
        for invoice in invoice_data:
            ...
            matrah_tl = float(
                invoice.get("matrah", 0) or invoice.get("toplam_tutar_tl", 0) or 0
            )
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (0, -1),
                        6,
                    ),  # Fatura no sütunu daha küçük font
                    ...
                    (
                        ...
                        (4, -1),
                        "LEFT",
                    ),  # Fatura no, tarih, firma, malzeme, miktar - sol
        ...
        # Özet bölümü ekle
        summary_section = self._create_table_summary(
            invoice_data, invoice_type, turkish_font, turkish_font_bold, lang
        )
    ...
    def _create_table_summary(
        self, invoice_data, invoice_type, turkish_font, turkish_font_bold, lang="tr"
    ):
        ...
        total_tl = sum(
            float(inv.get("matrah", 0) or inv.get("toplam_tutar_tl", 0) or 0)
        ...
        summary_data = [
            [
                f"{tr('summary_invoice_count', lang)}:",
                f"{count} {tr('unit_piece', lang)}",
                f"{tr('summary_total_tl', lang)}:",
                f"{total_tl:,.2f} {tr('currency_tl', lang)}",
            ],
            [
                f"{tr('summary_total_usd', lang)}:",
                f"{total_usd:,.2f} {tr('currency_usd', lang)}",
                f"{tr('summary_total_eur', lang)}:",
                f"{total_eur:,.2f} {tr('currency_eur', lang)}",
            ],
            [
                f"{tr('summary_total_vat', lang)}:",
                f"{total_kdv:,.2f} {tr('currency_tl', lang)}",
                f"{tr('summary_average', lang)}:",
                f"{average_tl:,.2f} {tr('currency_tl', lang)}",
            ],
        ]
        ...
        summary_story.append(
            Paragraph(
                f"<i>{tr('report_date', lang)}: {current_date}</i>",
                ParagraphStyle(
                    "DateInfo",
                    fontName=turkish_font,
                    fontSize=7,
    ...
    def _create_expense_document_layout(self, expense_data, lang="tr"):
        ...
        story.append(
            Paragraph(
                f"<b>{tr('excel_sheet_general_expenses', lang).upper()}</b>",
                ParagraphStyle(
                    "TableTitle",
                    fontName=turkish_font_bold,
                    fontSize=12,
    ...
    def _create_summary_section(self, invoice_data, lang="tr"):
        ...
        if len(currency_breakdown) > 1:
            ...
            for currency, amount in currency_breakdown.items():
                summary_data.append(
                    [f"{tr('summary_total', lang)} {currency}", f"{amount:,.2f} TL"]
                )
...
def export_monthly_income_to_pdf(
    year, monthly_results, quarterly_results, summary, file_path, lang="tr"
):
    ...
    try:
        ...
        title = Paragraph(
            f"<b>{year} {tr('report_title_suffix', lang)}</b>",
            ParagraphStyle(
                "Title",
                fontName=turkish_font_bold,
                fontSize=18,
        ...
        for i, month_name in enumerate(months):
            ...
            if i % 3 == 0:  # Start of quarter
                ...
                if q_index < len(quarterly_results):
                    odenecek_vergi = (
                        f"{quarterly_results[q_index].get('odenecek_kv', 0):,.2f} TL"
                    )
        ...
        # Tabloyu oluştur
        table = Table(
            table_data, colWidths=[3 * cm, 4 * cm, 4 * cm, 3.5 * cm, 4 * cm, 4.5 * cm]
        )
        ...
        for i in range(12):
            ...
            table_styles.append(
                ("TEXTCOLOR", (0, kdv_row), (-1, kdv_row), colors.HexColor("#666666"))
            )
        ...
        for q in range(4):
            ...
            table_styles.append(
                ("FONTNAME", (5, start_row), (5, end_row), turkish_font_bold)
            )
...
def export_monthly_general_expenses_to_pdf(
    expense_data, year=None, file_path=None, lang="tr"
):
    ...
    try:
        ...
        story.append(
            Paragraph(
                f"<b>{year} {tr('excel_sheet_general_expenses', lang).upper()} ({tr('col_months', lang)})</b>",
                title_style,
            )
        )
        ...
        # Tutar satırı
        amount_row = [tr("total", lang).upper()] + [
            f"{monthly_totals[i + 1]:,.2f} ₺" for i in range(12)
        ]
        ...
        story.append(
            Paragraph(
                f"<b>{tr('yearly_general_expenses', lang).upper()}: {total:,.2f} ₺</b>",
                total_style,
            )
        )
